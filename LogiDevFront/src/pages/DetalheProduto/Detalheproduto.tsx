...
export function DetalheProduto() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Garante que categoria e fornecedor tenham id numérico válido
        if (!produtoData.categoria || typeof produtoData.categoria !== 'object') {
          produtoData.categoria = { id: Number(produtoData.categoriaId), nome: '' };
        } else {
          produtoData.categoria.id = Number(produtoData.categoria.id);
        }
        if (!produtoData.fornecedor || typeof produtoData.fornecedor !== 'object') {
          produtoData.fornecedor = { id: Number(produtoData.fornecedorId), nome: '' };
        } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!produto) return;
    // Usa o id do objeto categoria/fornecedor, ou fallback para categoriaId/fornecedorId original do produto
    ...
    console.log('Payload enviado para o backend:', updatedDetails);
    try {
